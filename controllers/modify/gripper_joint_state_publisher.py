...
class GripperJointStatePublisher(object):
    ...
    def __init__(self, robot, jointPrefix):
        ...
        for name in GripperJointStatePublisher.jointNames:
            self.motors.append(robot.getMotor(name))
            print(robot.getMotor(name))
    ...
    def publish(self):
        ...
        msg.name = [s + self.jointPrefix for s in GripperJointStatePublisher.jointNames]
        msg.position = []
