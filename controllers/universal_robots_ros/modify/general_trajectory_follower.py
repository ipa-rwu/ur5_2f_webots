...
def interp_cubic(p0, p1, t_abs, joint_num):
...
def sample_trajectory(trajectory, t, joint_num):
    ...
    while trajectory.points[i + 1].time_from_start.to_sec() < t:
        i += 1
    return interp_cubic(trajectory.points[i], trajectory.points[i + 1], t, joint_num)
