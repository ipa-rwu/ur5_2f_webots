...
class TrajectoryFollower(object):
    ...
    def __init__(self, robot, jointStatePublisher, jointPrefix,gripperjointStatePublisher, gripperjointPrefix, goal_time_tolerance=None):
        self.robot = robot
        ...
        self.gripperjointPrefix = gripperjointPrefix
        self.gripperprefixedJointNames = [s + self.gripperjointPrefix for s in TrajectoryFollower.gripperjointNames]               
        self.grippermotors = []
        self.grippersensors = []            
        for name in TrajectoryFollower.gripperjointNames:
            self.grippermotors.append(robot.getMotor(name))
            self.grippersensors.append(robot.getPositionSensor(name + '_sensor'))
            self.grippersensors[-1].enable(self.timestep) 
            
        self.goal_handle_gripper = None
        self.last_point_sent_gripper = True
        self.trajectory_gripper = None
        self.joint_goal_tolerances_gripper = [0.05]                                                        
    ...
    def on_goal_gripper(self, goal_handle_gripper):
        ...
        # Orders the joints of the trajectory according to joint_names
        reorder_trajectory_joints(goal_handle_gripper.get_goal().trajectory, self.gripperprefixedJointNames)
    ...
    def on_cancel_gripper(self, goal_handle_gripper):
        ...
        if goal_handle_gripper == self.goal_handle_gripper:
            # stop the motors
            for i in range(len(TrajectoryFollower.jointNames)):
                self.grippermotors[i].setPosition(self.sensors[i].getValue())
    ...
    def update(self):
        if self.robot and self.trajectory:
            now = self.robot.getTime()
            if (now - self.trajectory_t0) <= self.trajectory.points[j-1].time_from_start.to_sec():  # Sending intermediate points
    ...
    def update_gripper(self):
        if self.robot and self.trajectory_gripper:
            ...
            if (now - self.trajectory_gripper_t0) <= self.trajectory_gripper.points[-1].time_from_start.to_sec():  # Sending intermediate points
                ...
                for i in range(len(setpoint_gripper.positions)):
                    self.grippermotors[i].setPosition(setpoint_gripper.positions[i])
                    # Velocity control is not used on the real robot and gives bad results in the simulation
                    # self.motors[i].setVelocity(math.fabs(setpoint.velocities[i]))
